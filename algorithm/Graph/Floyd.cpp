...
适用:有向网,无向网
存储结构:邻接矩阵
功能:求出每一对顶点之间的最短路径(劝之和最小),其中权值可以为负数,并可检测负权回路
*/
#include<stdio.h>
...
int Path[MVNum][MVNum];//辅助二维数组Path,Path[i][j],用于记录最短路径上顶点(下标为j)的前一顶点的序号
int D[MVNum][MVNum];//辅助二维数组D,D[i][j],用于记录顶点(下标为i)和顶点(下标为j)之间的最短路径长度
